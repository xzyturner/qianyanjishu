...
class SurroundOcc_v1(MVXTwoStageDetector):
    def __init__(self,
                 use_grid_mask=False,
    ...
    def generate_output(self, pred_occ, img_metas):
        ...
        for i in range(voxel.shape[0]):
            ...
            # vertices[:, 0] = (vertices[:, 0] + 0.5) * (img_metas[i]['pc_range'][3] - img_metas[i]['pc_range'][0]) / \
            #                  img_metas[i]['occ_size'][0] + img_metas[i]['pc_range'][0]
            # vertices[:, 1] = (vertices[:, 1] + 0.5) * (img_metas[i]['pc_range'][4] - img_metas[i]['pc_range'][1]) / \
            #                  img_metas[i]['occ_size'][1] + img_metas[i]['pc_range'][1]
            # vertices[:, 2] = (vertices[:, 2] + 0.5) * (img_metas[i]['pc_range'][5] - img_metas[i]['pc_range'][2]) / \
            #                  img_metas[i]['occ_size'][2] + img_metas[i]['pc_range'][2]
            # vertices[:, 0] = 1.5 * vertices[:, 0] - img_metas[i]['pc_range'][0]
            # vertices[:, 1] = 1.5 * vertices[:, 1] - img_metas[i]['pc_range'][1]
            # vertices[:, 2] = 1.5 * vertices[:, 2] - img_metas[i]['pc_range'][2]
            # ?
            # vertices[:, 0] = (vertices[:, 0] + 90 * 1.5) / 1.5
            # vertices[:, 1] = (vertices[:, 1] + 0) / 1.5
            # vertices[:, 2] = (vertices[:, 2] + 90 * 1.5) / 1.5
            ...
            save_dir = os.path.join('/mnt/d/project/SurroundOcc-main/visual_dir//visual_dir',
                                    img_metas[i]['occ_path'].replace('.npy', '').split('/')[-1])
            os.makedirs(save_dir, exist_ok=True)

            o3d.io.write_point_cloud(os.path.join(save_dir, 'pred.ply'), pcd)
            np.savetxt(os.path.join(save_dir, 'pred.txt'), vertices)
