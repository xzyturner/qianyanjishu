...
def evaluation_reconstruction(pred_occ, gt_occ, img_metas):
    ...
    for i in range(pred_occ.shape[0]):
        ...
        # print(vertices_pred.type())
        # print(vertices_gt.type())
...
def evaluation_semantic(pred_occ, gt_occ, img_metas, class_num):
    results = []
    pred = pred_occ.cpu().numpy()
    # np.save("/mnt/d/project/SurroundOcc-main/save//"+img_metas["occ_path"].split("//")[-1].split(".")[0]+".npy",pred)
    # print(img_metas["occ_path"].split("//")[-1].split(".")[0])
    for i in range(pred_occ.shape[0]):
